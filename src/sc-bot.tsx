...
export default function ScBot() {
  ...
  // Debug: cek komponen load
  useEffect(() => {
    console.log('ScBot loaded');
    console.log('Files:', files);
  }, []);

  // Fetch ukuran file saat tombol diklik
  const handleOpen = async (i: number, file: FileItem) => {
    setOpenIdx(i);
    if (!sizes[file.name]) {
      try {
        // path sudah diperbaiki ke public folder
  ...
  return (
    <div style={{
      ...
      />
      <h1 style={{ color: '#5b54a1', fontSize: 24, margin: '8px 0' }}>
        Anime Download Link Tree
      </h1>
      <p style={{ color: '#555', marginBottom: 26 }}>
        Pilih file ZIP yang ingin kamu unduh:
      </p>
              ...
              <div style={{
                ...
                <p>
                  ...
                  <span style={{ color: '#888', fontSize: 13 }}>
                    Ukuran: {sizes[f.name] || 'Unknown'}, Format: ZIP
                  </span>
                </p>
                <a
                  href={`/sc/${f.name}`}
